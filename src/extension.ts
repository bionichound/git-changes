...
const FLAG = CONF.get<string>("enabled");
const OWNER = CONF.get<string>("owner");
const REPO = CONF.get<string>("repo");
const CHANGE_FILE_NAMES: string[] = ["CHANGES.md", "CHANGELOG.md"];
var CHANGE_BLOCK = new RegExp(/^[#]+ \[\S+\] \- \S+$/);
var VERSION = new RegExp(/\[(?<version>\S+)\](?:[^:])/);
var LDECOMPOSE = new RegExp(/\[(?<repoMatch>\S+) (?<v1>\S+) +\-\> +(?<v2>\S+)\](?:[^:])/);

function is_changes(fileName: string): boolean {
    var name: string;
...
function getLatestBlock(text: string): [string, string, string] {
    var inBlock = false;
...
function buildVersion(match: string): string {
    return `https://github.com/${OWNER}/${REPO}/releases/tag/${match}`;
}

function buildLink(match: RegExpExecArray): string {
    if (!match.groups) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (match != null) {
            ...
            if (match.groups) {
                ...
                links.push(
                    match[0].trimEnd() + ":" + buildVersion(match.groups["version"])
                );
            }
        }
...
function redefineBlock(block: string, links: string): string {
    block = block.trimEnd();
    var lines = block.split("\n");
    var newBlock: string[] = [];
    for (let i = 0; i < lines.length; i++) {
            ...
            vscode.workspace.onDidSaveTextDocument((document: vscode.TextDocument) => {
                if (
                    ...
                    vscode.workspace.fs.writeFile(
                        document.uri,
                        Buffer.from([before, block, after].join("\n"))
                    );
                }
            })
        );
